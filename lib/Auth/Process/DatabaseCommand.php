...
class DatabaseCommand
{
    ...
    private function writeLoginIp($sourceIdp, $service, $user, $ip, $date)
    {
        ...
        if(empty($params['ip'])) {
            SimpleSAML\Logger::error("[proxystatistics] Couldn't find ip for storing information to table");
            return false;
        }
        if ($this->dbDriver == 'pgsql') {
    ...
    public function insertLogin(&$request, &$date)
    {
        ...
        if (empty($idpEntityID) || empty($spEntityId)) {
            ...
            if ($this->writeLoginIp($idpEntityID, $spEntityId, $userId, $ip, $dateTimestamp) === false) {
                SimpleSAML\Logger::error("The login log for ip wasn't inserted into table: " . $this->ipStatisticsTableName . ".");
            }
            if (!empty($idpName)) {
                if ($this->dbDriver == 'pgsql') {
